     ...
     (let loop ()
       (match (thread-receive)
         ([batch-id batch batch-completion hooks pending-transactions]
          (displayln db-manager_loop: " " batch-id " " batch " " batch-completion " " hooks " " pending-transactions)
          ...
          (displayln db-manager_loop1: " " batch-id " " batch-completion)
          (completion-post! batch-completion batch-id)
          (displayln db-manager_loop2: " " batch-id " " batch-completion)
          (with-db-lock (c)
            (if (and (DbConnection-triggered? c) (zero? (hash-length (DbConnection-open-transactions c))))
              (finalize-batch! c)
              (set! (DbConnection-ready? c) #t)))
          (displayln db-manager_loop3: " " batch-id " " batch-completion)
          (loop))
         (#f (void))
         (x (error "foo" x)))))))
