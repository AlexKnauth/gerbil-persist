...
(def (get-persistent-variable pvar)
  (DDT get-persistent-variable_open-lock: Any pvar)
  (def result (with-lock (persistent-variable-mx pvar)
    (lambda ()
  ...
  (DDT get-persistent-variable_close-lock: Any result)
  result)

(def (get-persistent-variable-set! pvar val)
  (DDT get-persistent-variable-set!_open-lock: Any pvar Any val)
  (def result (with-lock (persistent-variable-mx pvar)
    (lambda ()
      (with-tx (tx) (db-put! (persistent-variable-key pvar) val tx))
      (set! (persistent-variable-loaded? pvar) #t)
      (set! (persistent-variable-value pvar) val))))
  (DDT get-persistent-variable-set!_close-lock: Any pvar Any val)
  result)
